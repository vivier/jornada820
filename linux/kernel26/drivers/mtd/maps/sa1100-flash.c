 ...
 * 
 * Jornada820 version based on sa1100-flash.c 1.10 from cvs.handhelds.org
 * $Id: sa1100-flash.c,v 1.1 2004/06/24 16:58:37 fare Exp $
 */
...
static int __init sa1100_static_partitions(struct mtd_partition **parts)
{
	...
	if (machine_is_jornada820()) {
		*parts = jornada820_partitions;
		...
		sa1100_probe_map.size = JORNADA820_FLASH_SIZE;
		base=0x00000000;
	}
#endif
#ifdef CONFIG_SA1100_PANGOLIN
...
static void __init sa1100_probe_one_cs(unsigned int msc, unsigned long phys)
{
	...
	if (!mtd)
	 mtd = do_map_probe("map_rom", &sa1100_probe_map);
#endif
