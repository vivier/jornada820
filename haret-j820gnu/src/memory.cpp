...
uint8 *memPhysMap (uint32 paddr)
{
  ...
  if (slot == -1)
  {
    ...
    VirtualFree (phys_mem [slot], 0, MEM_DECOMMIT);
  ...
  if (phys_base [0] == 1)
  {
    ...
    {
      VirtualFree (phys_mem [0], 0, MEM_DECOMMIT);
...
void memPhysReset ()
{
  for (int i = 0; i < PHYS_CACHE_COUNT; i++)
    if (phys_mem [i])
    {
      VirtualFree (phys_mem [i], 0, MEM_DECOMMIT);
...
uint8 *memPhysMap (uint32 paddr)
{
  if (!pmInited)
  {
    ...
    if (!pmWindow)
    {
      Complain (C_ERROR ("Failed (%d) to alloc a virtual address window (size=%d)"),
                GetLastError (),(PHYS_CACHE_COUNT + 1) * 64 * 1024);
                return NULL;
    }
                      ...
                      PAGE_READWRITE | PAGE_PHYSICAL | PAGE_NOCACHE))
    {
      Complain (C_ERROR ("Failed (%d) to map a dummy memory area to virtual address window"),
                GetLastError ());
err:  
	VirtualFree (pmWindow, 0, MEM_DECOMMIT);
                      ...
                      PAGE_READONLY | PAGE_PHYSICAL | PAGE_NOCACHE))
    {
      Complain (C_ERROR ("VirtualCopy() failed (%d) when accessing MMU L1 table"),
                GetLastError ());
      VirtualFree (l1desc, 0, MEM_DECOMMIT);
                      ...
                      PAGE_READWRITE | PAGE_PHYSICAL))
    {
      Complain (C_ERROR ("VirtualCopy() failed (%d) when accessing MMU L2 table"),
                GetLastError ());
      VirtualFree (pmL2PT, 0, MEM_DECOMMIT);
...
//hack to identify the machine we're running on
void machine_id(void)
{
  uint32 magic[3];

#if 0
  magic[0]=memPhysRead(0x40);
  magic[1]=memPhysRead(0x44);
  magic[2]=memPhysRead(0x80);
#else
  magic[0]=*(unsigned long *)(0xa0000040);
  magic[1]=*(unsigned long *)(0xa0000044);
  magic[2]=*(unsigned long *)(0xa0000080);
#endif

  Output(L"Machine ID:\nMagic values: 40:0x%.8x 44:0x%.8x 80:0x%.8x\n",magic[0],magic[1],magic[2]);

  if (magic[0]==0xbe4c9208) Output(L"Jornada 820 us\n");
  if (magic[0]==0x43454345)
	  {
	  if (magic[1]==0x80289a80) Output(L"hpaq??\n");
	  if ((magic[1]==0x81627ae0)&&(magic[2]==0x8cac7fdc)) Output(L"Jornada 720 us\n");
										          if ((magic[1]==0x80297604)&&(magic[2]==0x7125d68e)) Output(L"Jornada 56x ??\n");
											 }
