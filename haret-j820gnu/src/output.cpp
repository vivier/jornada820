...
#define CreateDialogParamW(hInstance, lpszDlgName, hwndOwner, lpfnDialog, lParam) \
    CreateDialogIndirectParamW(hInstance,  \
        (const DLGTEMPLATE *)LoadResource(hInstance, FindResource(hInstance, lpszDlgName, RT_DIALOG)), \
...
void Log (const wchar_t *format, ...)
{
  ...
  *eol++ = L'\n'; *eol = 0;
  ...
  unsigned int maxlen = SendMessage (hConsole, EM_GETLIMITTEXT, 0, 0);

  unsigned int tl, sl = buff + (sizeof (buff) / sizeof (wchar_t)) - dst;

  while ((tl = GetWindowTextLength (hConsole)) + sl >= maxlen)
  {
    unsigned int linelen = SendMessage (hConsole, EM_LINELENGTH, 0, 0) + 2;
...
static BOOL pbDialogFunc (HWND hWnd, UINT message, WPARAM wParam,
  LPARAM lParam)
{
  switch (message)
  {
...
bool InitProgress (unsigned int Max)
{
#ifdef USE_WAIT_CURSOR
  OldCursor = (HCURSOR)-1;
#endif
...
bool SetProgress (unsigned int Value)
{
  if (!pb)
    return false;
