...
sa1100fb_get_machine_info(struct sa1100fb_info *fbi)
{
	...
	fbi->lm = lcd_device_find("sa1100fb");
	fbi->bm = backlight_device_find("sa1100fb");
	if (fbi->lm)
		/* Since lcd_device_find increments the reference counter
		 * on the module that contains the static sa1100fb_mach_info
		 * structure, we can be sure that the data pointed by it is
		 * valid until we call lcd_device_put().
		 */
		inf = class_get_devdata (&fbi->lm->class_dev);
#endif	
...
#ifdef CONFIG_LCD_DEVICE
	if (fbi->bm && fbi->bm->props->set_power)
		fbi->bm->props->set_power(fbi->bm, on ? 0 : 4);
#else
...
#ifdef CONFIG_LCD_DEVICE
	if (fbi->lm && fbi->lm->props->set_power)
		fbi->lm->props->set_power(fbi->lm, on ? 0 : 4);
#else
